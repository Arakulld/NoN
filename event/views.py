...
_QR_CODE_EXT = 'png'
QR_CODE_SAVE_NAME = '{user_id}_{event_id}.' + _QR_CODE_EXT


def generate_qr_code(data):
    qr_code = qrcode.make(data)
    qr_code_path = TEMP_PATH + 'temp'
...
def add_qr_code_to_participant(data, participant, user, event):
    ...
    participant.qr_code.save(QR_CODE_SAVE_NAME.format(user_id=user.pk, event_id=event.id), open(old_path, "rb"), True)
    os.remove(old_path)
...
def events_list(request):
    if 'display_my' in request.GET == 'participating':
